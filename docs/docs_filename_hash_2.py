...
def extract_pdfinfo_title(file_path):
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        # Using a regular expression to extract the content of \pdfinfo{...} /Title
        match = re.search(r'\\pdfinfo{.*?/Title\s*\((.*?)\)', content, re.DOTALL)
...
def generate_short_code(string):
    ...
    # Return the first 4 characters of the uppercase hash
    return sha256_hash_upper[:4]
...
with open('FILES_HASH_CODES.JSON', 'w') as file:
    sys.stdout = file
        ...
        for filename in filenames:
            if filename.endswith('.tex'):
                ...
                if title_content:
                    ...
                    file_info = {
                        "file_path": filename if args.keep_relative_path else file_path,
                        "pdf_title": title_content,
