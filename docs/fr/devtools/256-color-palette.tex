...
\title{\color{sec1}Fonctionnement du script\\ \color{sec2}256-color-palette.sh\color{sec1}}\color{text}
...
\begin{document}
    ...
    \begin{justify}
        Ce script affiche sur le terminal chaque code couleur d'une table de palette XTERM dans un tableau 6 * 40, avec la couleur correspondante de chaque code imprimée en arrière-plan.
    \end{justify}
    ...
    \begin{justify}
        En cas de passage de la valeur "-h" ou "--help" en premier argument, un simple message résumant de manière très simple le but du script s'affiche sur le terminal, sans que son code principal, écrit dans la boucle for ne s'exécute.
    \end{justify}
    ...
    \color{sec2}
    \subsection{Affichage de la table de palette XTERM}\color{text}
