...
## INITIALIZER FILE'S FILES

# Creating the initialization log file.
INITIALIZER_LOG_FILE_DIR="$BASH_UTILS_ROOT/tmp/$PROJECT_NAME/logs/"
INITIALIZER_LOG_FILE_PATH="$INITIALIZER_LOG_FILE_DIR/$PROJECT_NAME.init"

if [ ! -d "$INITIALIZER_LOG_FILE_DIR" ]; then
    mkdir -p "$INITIALIZER_LOG_FILE_DIR"
fi
...
function WriteInitLog
{
    ...
    if [ -n "$initlog_status" ]; then
        if [ "$initlog_status" = "log" ]; then
            echo "$string" >> "$INITIALIZER_LOG_FILE_PATH"
...
#### DEFINING THE PROJECT SCRIPT'S DIRECTORY PATH AND ITS FILES
...
lineno=$LINENO; if [ -z "$PROJECT_PATH" ]; then
    echo "In ${BASH_SOURCE[0]}, line $lineno : No path provided in the PROJECT_PATH variable in $PROJECT_PATH"; exit 1
lineno=$LINENO; elif [ ! -f "$PROJECT_PATH" ]; then
    echo "In ${BASH_SOURCE[0]}, line $lineno : Incorrect path provided as value for the PROJECT_PATH variable in $PROJECT_PATH";
...
PROJECT_LOG_FILE_NAME="$PROJECT_NAME $DATE_TIME.log"
PROJECT_LOG_FILE_PATH="$INITIALIZER_LOG_FILE_DIR/$PROJECT_LOG_FILE_NAME"
...
WriteInitLog "In ${BASH_SOURCE[0]}, line $LINENO : LINKING BASH-UTILS LIBRARY FILES TO $PROJECT_NAME" \
...
#### END OF THE INITALIZER FILE

WriteInitLog
WriteInitLog "$(DrawLine "$COL_RESET" "-")"; WriteInitLog "END OF INITIALIZATION PROCESS, PROCESSING $(Decho "${PROJECT_NAME^^}") NOW"; WriteInitLog "$(DrawLine "$COL_RESET" "-")"
WriteInitLog; WriteInitLog
