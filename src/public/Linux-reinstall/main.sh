...
1) Get script name and store it in MAIN_SCRIPT_NAME
2) Set $MAIN_PROJECT_ROOT path as /usr/local/bin/Bash-Utils/src/public/$MAIN_SCRIPT_NAME
...
# General library related directories path
MAIN_L_FNCTS="lib/functions"
MAIN_L_LANG="lang"
MAIN_L_VARS="lib/variables"

# Project related directories path
...
{ echo "Project root directory path : $MAIN_PROJECT_ROOT"; echo; } > "$MAIN_LOG"
...
{ echo "Project root directory name : $MAIN_PROJECT_RESULT"; echo; } >> "$MAIN_LOG"
...
if test "$?" -eq 0; then echo "$MAIN_PROJECT_ROOT/$MAIN_S_LANG/SetMainLang.sh : Unable to find the main file's variables translation file" && exit 1; fi
echo "Included file : $MAIN_PROJECT_ROOT/$MAIN_S_LANG/SetMainLang.sh"

# shellcheck source="$MAIN_PROJECT_ROOT/$MAIN_LANG/SetLibLang.sh"
source "$MAIN_PROJECT_ROOT/$MAIN_LANG/SetLibLang.sh" \
if test "$?" -eq 0; then echo "$MAIN_PROJECT_ROOT/$MAIN_LANG/SetLibLang.sh : Unable to find the main file's variables translation file" && exit 1; fi
echo "Included file : $MAIN_PROJECT_ROOT/$MAIN_LANG/SetLibLang.sh"


# -----------------------------------------------

## VARIABLES AND FUNCTIONS FILES

# shellcheck source=$MAIN_PROJECT_ROOT/$MAIN_S_RES/main.var
source "$MAIN_PROJECT_ROOT/lib/link_fncts_vars.sh" && echo "Included variable file : $MAIN_S_VARS/main.var" 2>&1 | tee -a "$MAIN_LOG"
if test "$?" -eq 0; then echo "$MAIN_PROJECT_ROOT/lib/link_fncts_vars.sh : Unable to find the library linker file" && exit 1; fi
echo "Included file : $MAIN_PROJECT_ROOT/lib/link_fncts_vars.sh"
...
{    
	...
	if test "$EUID" -ne 0; then
		EchoErrorLog "$MSG_MAIN_INIT_ROOT_ZERO."
		EchoErrorLog "$MSG_MAIN_INIT_ROOT_ZERO_EXEC :"
		...
		echo "$MSG_MAIN_INIT_ARGS_SUDO"
		echo

		EchoErrorLog "$MSG_MAIN_INIT_ROOT_ZERO_OR_1,"
		EchoErrorLog "$MSG_MAIN_INIT_ROOT_ZERO_OR_2 :"
		echo "$MSG_MAIN_INIT_ARGS"
		echo

		EchoErrorLog "$MSG_MAIN_INIT_ROOT_FAIL !"
		EchoErrorLog "$MSG_MAIN_INIT_ROOT_ADVICE."
		echo
	...
	if test -z "$ARG_USERNAME"; then
		EchoErrorLog "$MSG_MAIN_INIT_USERNAME_ZERO :"
		echo "$MSG_MAIN_INIT_ARGS_SUDO"
		echo

		EchoErrorLog "$MSG_MAIN_INIT_USERNAME_FAIL !"
		EchoErrorLog "$MSG_MAIN_INIT_USERNAME_ADVICE."
		echo
	...
	# Else, if the username argument doesn't match to any existing user account.
		...
		EchoErrorLog "$MSG_MAIN_INIT_USERNAME_INCORRECT !"
		EchoErrorLog "$MSG_MAIN_INIT_USERNAME_INCORRECT_ADVICE."
		echo
	...
	if test -z "$ARG_INSTALL"; then
        ...
        EchoErrorLog "$MSG_MAIN_INIT_INSTALL_ZERO !"
        EchoErrorLog "$MSG_MAIN_INIT_INSTALL_ZERO_ADVICE."
        echo
        
        EchoErrorLog "$MSG_MAIN_INIT_INSTALL_AWAITED."
        echo
    ...
    # Else, if the second argument is passed, the script checks if the value is equal to only one of the awaited values ("custom" or "sio"). They are not case sensitive.
        ...
        case ${ARG_INSTALL,,} in
            ...
            *)
                ...
                EchoErrorLog "$MSG_MAIN_INIT_INSTALL_DIFFERENT !"
                EchoErrorLog "$MSG_MAIN_INIT_INSTALL_AWAITED."
                echo
