...
# Script's path
PROJECT_PATH="$(GetProjectParentPath)/$(basename "$0")"
echo "$PROJECT_PATH"
...
#
LINUX_REINSTALL_INST="$(GetProjectParentPath)/install/categories"   # TODO : Retirer ce dossier du template de fichier principal une fois le script de réinstallation fini
...
function CheckArgs()
{
	...
	if test "$EUID" -ne 0; then
		...
		EchoError "$MSG_MAIN_INIT_ROOT_ZERO_EXEC :"
		...
		EchoError "$MSG_MAIN_INIT_ROOT_ZERO_OR_1,"
		EchoError "$MSG_MAIN_INIT_ROOT_ZERO_OR_2 :"
		echo "$MSG_MAIN_INIT_ARGS"
		echo

		EchoError "$MSG_MAIN_INIT_ROOT_FAIL !"
		EchoError "$MSG_MAIN_INIT_ROOT_ADVICE."
		echo
	...
	if test -z "$ARG_USERNAME"; then
		...
		EchoError "$MSG_MAIN_INIT_USERNAME_FAIL !"
		EchoError "$MSG_MAIN_INIT_USERNAME_ADVICE."
		echo
...
echo
	# Else, if the username argument doesn't match to any existing user account.
		...
		EchoError "$MSG_MAIN_INIT_USERNAME_INCORRECT !"
		EchoError "$MSG_MAIN_INIT_USERNAME_INCORRECT_ADVICE."
		echo
