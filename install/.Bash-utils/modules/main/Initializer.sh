...
# Add value in the "$__BASH_UTILS_MAIN_INIT_STR_ARRAY" array that stores the initialization log output, according to the "$__BU_MAIN_STAT_PRINT_INIT_LOG" status variables's value :
#   "true"  --> store the text into the "$__BASH_UTILS_MAIN_INIT_STR_ARRAY" array AND display text to the terminal.
#   "false" --> store the text into the "$__BASH_UTILS_MAIN_INIT_STR_ARRAY" array WITHOUT displaying any text. 
function InitializerAddInitStrArrayVal()
{
    #***** Parameters *****
    p_string=$1             # String to store in the "$__BASH_UTILS_MAIN_INIT_STR_ARRAY" array.
    ...
    if [ "$__BU_MAIN_STAT_PRINT_INIT_LOG" = "true" ]; then
        case "$p_option" in
            '-n' | 'n')
                __BASH_UTILS_MAIN_INIT_STR_ARRAY+=("$p_string"); echo -ne "${p_string##*] }"    # Cutting the log entry's date from a string, before displaying it on the terminal.
                ;;
            '' | *)
                __BASH_UTILS_MAIN_INIT_STR_ARRAY+=("$p_string"); echo -e "${p_string##*] }"     # Cutting the log entry's date from a string, before displaying it on the terminal.
                ;;
            esac
    else
        __BASH_UTILS_MAIN_INIT_STR_ARRAY+=("$p_string")
    fi
}
...
# Sourcing each file listed into the "$__BASH_UTILS_MAIN_FUNCTIONS_FILES_PATH" CheckTxtFmt.

# shellcheck disable=SC1090
for f in "${__BASH_UTILS_MAIN_FUNCTIONS_FILES_PATH[@]}"; do
    source "$f" || ModuleInitializer_SourcingFailure "$f" "$(ModuleInitializer_GetModuleName "${BASH_SOURCE[0]}")"; __BASH_UTILS_MAIN_INIT_LIB_FILES_PATH_ARRAY+=("$f")
done
...
# shellcheck disable=SC1090
for f in "${__BASH_UTILS_MAIN_INIT_LIST_CONFIG_FILES_PATH_ARRAY[@]}"; do
    source "$f" || ModuleInitializer_SourcingFailure "$f" "$(ModuleInitializer_GetModuleName "${BASH_SOURCE[0]}")"; __BASH_UTILS_MAIN_INIT_LIB_FILES_PATH_ARRAY+=("$f")
done
...
if CheckIsInitializing; then
    # shellcheck disable=SC2034
    __BASH_UTILS_MAIN_INIT_IS_INITALIZING="false"
fi
