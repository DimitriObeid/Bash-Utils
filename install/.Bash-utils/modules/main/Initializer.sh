...
function BU.Main.Initializer.SourceConfig()
{
	...
	for f in "${__BU_MAIN_MODULE_LIST_CONFIG_FILES_PATH_ARRAY[@]}"; do
		BU.ModuleInit.IsFrameworkWrapped || source "$f" || { BU.ModuleInit.SourcingFailure "$f" "$(BU.ModuleInit.GetModuleName "${BASH_SOURCE[0]}")" "$(basename "${BASH_SOURCE[0]}")" "${FUNCNAME[0]}" "$LINENO"; __BU_MAIN_MODULE_CONFIG_FILES_PATH_ARRAY+=("$f"); v_loop_error='error'; break; };
