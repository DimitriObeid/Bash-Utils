...
function BU.Main.Echo.NewlineF() { local p_line_breaks=$1; if [[ "$p_line_breaks" =~ ^[\-0-9]+$ ]]; then PrintLog "$(BU.Main.Echo.Newline "$p_line_breaks")"; else PrintLog "$(BU.Main.Echo.Newline)"; fi; return 0; }
