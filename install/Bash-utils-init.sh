...
function BU.ModuleInit.IsFrameworkUnlocalizedCompiled()  { local v_currFile; v_currFile="$(basename "${BASH_SOURCE[0]}")"; if [[ "${v_currFile##*/,,}" == Bash-utils?(-stable)-full.sh ]] && [ "$(wc -l "${v_currFile}" | cut -f1 -d" ")" -ge 19078 ]; then echo "FILE = ${v_currFile}"; return 0; else return 1; fi }
