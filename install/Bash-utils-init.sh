...
{
    #**** Parameters ****
    local v_file=${1:-${BASH_SOURCE[0]}};
    local v_func=${2:-${FUNCNAME[0]}};
    local v_line=${3:-${LINENO}};
...
function BU.ModuleInit.DisplayInitGlobalVarsInfos()
{
    if [ "${__BU_MODULE_INIT_MSG_ARRAY_MODE}" == '--mode-log-full' ]; then
        ...
        local p_file=${6:-NULL};        # String    - Default : NULL    - File where the variable was declared.
        local p_func=${7:-NULL};        # String    - Default : NULL    - Function where the variable was declared.
        local p_line=${8:-NULL};        # Int       - Default : NULL    - Line where the variable was declared.
...
{
    #**** Parameters ****
    local p_file=${1:-$'\0'};       # String    - Default : NULL    - File where the error happened.
...
{
    #**** Parameters ****
    local p_module=${1:-$'\0'}; # Modules to include passed as argument, with its own parameters.
    local p_count=${2:-$'\0'};  # Counting the times the function was called in the "BashUtils_InitModules" function's main loop.
    ...
    if [ "${p_count}" -eq 0 ]; then
        ...
        if [[ "${p_module}" == "${v_module_name}" ]]; then
            ...
            {
                #**** Parameters ****
                local p_value=${1:-$'\0'};  # New value to assign to the "${__BU_MODULE_INIT_MSG_ARRAY_PERMISSION" global variable.
