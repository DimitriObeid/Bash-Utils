...
# Checking if the whole framework's code is wrapped in a single file (generated by another (future) script).
function BU::IsFrameworkWrapped() { if [ -n "$__BASHUTILS_FRAMEWORK_IS_WRAPPED_IN_A_SINGLE_FILE" ] && [ "$__BASHUTILS_FRAMEWORK_IS_WRAPPED_IN_A_SINGLE_FILE" = 'Framework wrapped' ]; then return 0; else return 1; fi }
