...
# shellcheck disable=
function BU.ModuleInit.IsFrameworkCompiledStable()      { local v_currFile; v_currFile="$(basename "${BASH_SOURCE[0]}")"; if [[ "${v_currFile##*/,,}" == bash-utils-stable?(-full|-[a-z][a-z]).sh ]]; then return 0; else return 1; fi }
...
# shellcheck disable=
function BU.ModuleInit.IsFrameworkCompiledUnstable()    { local v_currFile; v_currFile="$(basename "${BASH_SOURCE[0]}")"; if [[ "${v_currFile##*/,,}" == bash-utils?(-full|-[a-z][a-z]).sh ]]; then return 0; else return 1; fi }
...
# shellcheck disable=
function BU.ModuleInit.IsFrameworkCompiledLocalized()   { local v_currFile; v_currFile="$(basename "${BASH_SOURCE[0]}")"; if [[ "${v_currFile##*/,,}" == bash-utils?(-stable)-[a-z][a-z].sh ]] && [ "$(wc -l "${v_currFile}" | cut -f1 -d" ")" -ge 15000 ]; then return 0; else return 1; fi }
...
function BU.ModuleInit.SIGINT()
{
    ...
    [ "${__BU_MODULE_INIT__USER_LANG,,}" == 'id' ] && { echo >&2; echo "Eksekusi skrip diinterupsi oleh pengguna" >&2 && v_isPrinted='true'; }
    ...
    [ "${__BU_MODULE_INIT__USER_LANG,,}" == 'uk' ] && { echo >&2; echo "Виконання скрипта було перервано користувачем" >&2 && v_isPrinted='true'; }
...
if [ -z "${BASH_VERSION}" ]; then
    ...
    [ "$(echo "${LANG}" | cut -d _ -f1)" == 'id' ] && echo "BASH-UTILS ERROR: Interpreter shell Anda saat ini bukanlah interpreter « Bash », tetapi interpreter « ${SHELL##*/} »" >&2 && echo >&2;
    ...
    [ "$(echo "${LANG}" | cut -d _ -f1)" == 'uk' ] && echo "ВИНИКЛА ПОМИЛКА BASH-UTILS: Поточний інтерпретатор командного рядка - це не інтерпретатор « Bash », а інтерпретатор « ${SHELL##*/} »" >&2 && echo >&2;
