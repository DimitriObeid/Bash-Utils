...
# Checking if the whole framework's code (initializer and main module's code) is wrapped in a single file (generated by the "res/dev-tools/dev-bin/lib-unite.sh" (or "bin/lib-unite.sh") script).
function BU.ModuleInit.IsFrameworkWrapped() { local v_currFile="$(basename "${BASH_SOURCE[0]}")"; if [ "$v_currFile" == "Bash-utils.sh" ] && [ "$(wc -l "$v_currFile" | cut -f1 -d" ")" -ge 19078 ]; then echo; return 0; else return 1; fi }
...
function BU.ModuleInit.DefineBashUtilsGlobalVariablesBeforeInitializingTheModules()
{
    ...
    if ! BU.ModuleInit.IsFrameworkWrapped && [ -d "${__BU_MODULE_INIT__ROOT_HOME}/.Bash-utils" ]; then
