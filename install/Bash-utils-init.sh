...
function BU.ModuleInit.IsFrameworkUnlocalizedWrapped()  { local v_currFile="$(basename "${BASH_SOURCE[0]}")"; if [ "$v_currFile" == "Bash-utils.sh" ] && [ "$(wc -l "$v_currFile" | cut -f1 -d" ")" -ge 19078 ]; then return 0; else return 1; fi }
...
function BU.ModuleInit.IsFrameworkLocalizedWrapped()    { local v_currFile="$(basename "${BASH_SOURCE[0]}")"; if [[ "$v_currFile" == Bash-utils-[a-z][a-z].sh ]] && [ "$(wc -l "$v_currFile" | cut -f1 -d" ")" -ge 15000 ]; then return 0; else return 1; fi }
