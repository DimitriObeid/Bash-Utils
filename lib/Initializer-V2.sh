...
# Sourcing the "~/.bash_profile" file to call the "$__BASH_UTILS_MAIN_ROOT" CheckTxtFmt.
...
# As the "$__BASH_UTILS_MAIN_ROOT" CheckTxtFmt is defined, it's possible to source the initializer's configuration file.
...
function EchoInit
{
    ...
    if [ -z "$__BASH_UTILS_MAIN_INIT__LIST_FILE_PATH" ] || [ ! -f "$__BASH_UTILS_MAIN_INIT__LIST_FILE_PATH" ]; then
        ...
        if [ -z "$p_colorCode" ]; then
            __EchoInit "$p_str${__BU_MAIN_COLOR_RESET}"
        else
            if [[ "$p_colorCode" =~ [0-9] ]]; then
                __EchoInit "$(tput setaf "$p_colorCode")$p_str${__BU_MAIN_COLOR_RESET}"
            else
...
function InitErrMsg
{
    ...
    #***** Code *****
    echo >&2; echo -e "${__BU_MAIN_COLOR_ERROR}BASH-UTILS ERROR : In ${__BU_MAIN_COLOR_HIGHLIGHT}$(basename "${BASH_SOURCE[0]}")${__BU_MAIN_COLOR_ERROR}, line ${__BU_MAIN_COLOR_HIGHLIGHT}$p_lineno${__BU_MAIN_COLOR_ERROR} --> $p_msg${__BU_MAIN_COLOR_RESET}" >&2; echo >&2
...
function CheckBashMinimalVersion
{
	if [ "${BASH_VERSINFO[0]}" -lt 4 ]; then
		echo -ne "${__BU_MAIN_COLOR_ERROR}BASH-UTILS ERROR : In ${__BU_MAIN_COLOR_HIGHLIGHT}$(basename "${BASH_SOURCE[0]}")${__BU_MAIN_COLOR_ERROR}, line ${__BU_MAIN_COLOR_HIGHLIGHT}$(( LINENO-1 ))${__BU_MAIN_COLOR_ERROR} --> ${__BU_MAIN_COLOR_RESET}" >&2
		echo -e "${__BU_MAIN_COLOR_ERROR}This Bash library requires at least the Bash version ${__BU_MAIN_COLOR_HIGHLIGHT}4.0.0${__BU_MAIN_COLOR_RESET}" >&2
		echo >&2

		echo -e "${__BU_MAIN_COLOR_ERROR}Your Bash version is : ${__BU_MAIN_COLOR_HIGHLIGHT}$BASH_VERSION${__BU_MAIN_COLOR_RESET}" >&2
		echo >&2

		echo -e "${__BU_MAIN_COLOR_ERROR}Please install at least the ${__BU_MAIN_COLOR_HIGHLIGHT}4.0.0${__BU_MAIN_COLOR_ERROR} Bash version to use this library${__BU_MAIN_COLOR_RESET}"
		echo >&2
...
function CheckBURequirements
{
    ...
    if [ -d "$p_path" ]; then
        EchoInit "Found directory : ${__BU_MAIN_COLOR_HIGHLIGHT}$p_path${__BU_MAIN_COLOR_RESET}"

    # Else, if the path points towards a file.
    elif [ -f "$p_path" ]; then
        EchoInit "Found file : ${__BU_MAIN_COLOR_HIGHLIGHT}$p_path${__BU_MAIN_COLOR_RESET}"
    else
        InitErrMsg "The following path is incorrect : ${__BU_MAIN_COLOR_HIGHLIGHT}$p_path" "$p_lineno" "1"
    fi
}
...
function SourceDependency
{
    ...
    #***** Code *****
    [[ -e "$p_dep" ]] || InitErrMsg "This dependency file doesn't exists : ${__BU_MAIN_COLOR_HIGHLIGHT}$p_dep" "$LINENO" "1"

    # shellcheck disable=SC1090
    source "$p_dep" || InitErrMsg "Unable to source this dependency file : ${__BU_MAIN_COLOR_HIGHLIGHT}$p_dep" "$LINENO" "1"

    EchoInit "Sourced file : ${__BU_MAIN_COLOR_HIGHLIGHT}$p_dep${__BU_MAIN_COLOR_RESET}"
}
...
if [ -f "$__BASH_UTILS_MAIN_INIT__LIST_FILE_PATH" ]; then
    ...
    if [ ! -d "$__BU_MAIN_PROJECT_LOG_DIR_PATH" ]; then
        mkdir -p "$__BU_MAIN_PROJECT_LOG_DIR_PATH" || {
            echo >&2
            echo -e "${__BU_MAIN_COLOR_ERROR}In ${__BU_MAIN_COLOR_HIGHLIGHT}$(basename "${BASH_SOURCE[0]}")${__BU_MAIN_COLOR_ERROR}, line ${__BU_MAIN_COLOR_HIGHLIGHT}$(( LINENO-2 ))${__BU_MAIN_COLOR_ERROR} --> Error : unable to create the project's temporary directory and/or the project's logs directory.${__BU_MAIN_COLOR_RESET}" >&2; echo >&2; exit 1
        }
    fi
...
EchoInit "Sourcing the CheckTxtFmts status file :"; SourceDependency "$__BASH_UTILS_MAIN_CONF_PROJECT_STATUS"; EchoInit
...
__BU_MAIN_PROJECT_PATH="$(GetParentDirectoryPath "$0")/$__BU_MAIN_PROJECT_FILE_PATH"
...
# shellcheck disable=SC2034
__BU_MAIN_STAT_DEBUG="true";          CheckSTAT_DEBUG         "$(basename "${BASH_SOURCE[0]}")" "$LINENO"

# The function "CheckSTAT_LOG()" creates the log file and its path.
# shellcheck disable=SC2034
__BU_MAIN_STAT_LOG="true";            CheckSTAT_LOG           "$(basename "${BASH_SOURCE[0]}")" "$LINENO"

# shellcheck disable=SC2034
__BU_MAIN_STAT_LOG_REDIRECT="tee";    CheckSTAT_LOG_REDIRECT  "$(basename "${BASH_SOURCE[0]}")" "$LINENO"

# shellcheck disable=SC2034
__BU_MAIN_STAT_ERROR="fatal";         CheckSTAT_ERROR         "$(basename "${BASH_SOURCE[0]}")" "$LINENO"

# shellcheck disable=SC2034
__BU_MAIN_STAT_TIME_TXT="0";          CheckSTAT_TIME_TXT      "$(basename "${BASH_SOURCE[0]}")" "$LINENO"
...
if [ ! -d "$__BU_MAIN_PROJECT_TMP_PATH" ]; then
	EchoInit "$(mkdir -p "$__BU_MAIN_PROJECT_TMP_PATH")"
fi

CheckProjectRelatedFile "$__BU_MAIN_PROJECT_LOG_FILE_PATH"
CheckProjectRelatedFile "$__BU_MAIN_PROJECT_COLOR_CODE_FILE_PATH"
...
if [ ! -f "$__BU_MAIN_PROJECT_TR_FILE_PATH" ]; then
	touch "$__BU_MAIN_PROJECT_TR_FILE_PATH" || { InitErrMsg "Unable to create the ${__BU_MAIN_COLOR_HIGHLIGHT}$__BU_MAIN_PROJECT_TR_FILE_PATH file" "1"; }
	
	EchoInit "Created file : ${__BU_MAIN_COLOR_HIGHLIGHT}$__BU_MAIN_PROJECT_TR_FILE_PATH${__BU_MAIN_COLOR_RESET}"
else
	EchoInit "Found file : ${__BU_MAIN_COLOR_HIGHLIGHT}$__BU_MAIN_PROJECT_TR_FILE_PATH${__BU_MAIN_COLOR_RESET}"
fi
