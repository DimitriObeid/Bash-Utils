...
# Checking if the "" value is passed as default language for the comments generated by the "LibCompilerV4.Functions.WriteCommentCode.Heredoc.${LANGUAGE}()" function into the compiled file.
...
# Counter to increase for the "LibCompilerV4.CompileInSingleFile()" function's loop iterations.
...
# Path to the file which stores the content of the here document used into each of the "LibCompilerV4.Functions.WriteCommentCode.Heredoc.${__v_loop_curr_locale}()" functions, which is called into the "LibCompilerV4.Functions.WriteCommentCode()" function.
...
# shellcheck disable=
function LibCompilerV4.Functions.PrintLine()
{
    #**** Parameters ****
...
# shellcheck disable=
function LibCompilerV4.Functions.PrintBaseLine()
{
    #**** Parameters ****
    ...
    [[ ("${p_pos^^}" == 'FULL') || ("${p_pos^^}" == 'UPPER') ]] && {
        ...
        if [[ "${p_len,,}" == len?(g?(th)) ]]; then
            LibCompilerV4.Functions.PrintLine "${p_pos}" "${p_len}" "${p_msg}";

        else
            LibCompilerV4.Functions.PrintLine "${p_pos}";
        fi;
...
# shellcheck disable=
function LibCompilerV4.Functions.PrintErrorLine()   { LibCompilerV4.Functions.PrintBaseLine "${__ERROR}" "${1}" "${2}" "${3}" >&2; }

# ························
# Printing a newstep line.

# shellcheck disable=
function LibCompilerV4.Functions.PrintNewstepLine() { LibCompilerV4.Functions.PrintBaseLine "${__NEWSTEP}" "${1}" "${2}" "${3}"; }

# ························
# Printing a success line.

# shellcheck disable=
function LibCompilerV4.Functions.PrintSuccessLine() { LibCompilerV4.Functions.PrintBaseLine "${__SUCCESS}" "${1}" "${2}" "${3}"; }

# ························
# Printing a warning line.

# shellcheck disable=
function LibCompilerV4.Functions.PrintWarningLine() { LibCompilerV4.Functions.PrintBaseLine "${__WARNING}" "${1}" "${2}" "${3}" >&2; }
...
# shellcheck disable=SC2059
function LibCompilerV4.Functions.PrintFilesWhichWereNotChmoded()
{
    #**** Parameters ****
...
# ·············································································································································································
# Erasing every pieces of code which prevent the direct execution of their host files, since a new one is written in the "LibCompilerV4.Functions.WriteCommentCode()" function.

# shellcheck disable=
function LibCompilerV4.Functions.EraseSafeguardExecLines()
{
    sed -i "/if \[ \"\${0##\/*}\" == \"\${BASH_SOURCE[0]##\/*}\" \]; then/,/fi; exit 1; fi/d" "${__v_loop_compiled_file_path}";
...
# LOCALIZED : $(LibCompilerV4.CompileInSingleFile.Loop.IsCurrentLanguageEnglish && printf 'YES' || printf 'NO')
...
EOF
    ...
    LibCompilerV4.Functions.WriteCommentCode.Heredoc."${__v_loop_curr_locale}";
...
# shellcheck disable=
function LibCompilerV4.Functions.EraseComments()
{
...
# shellcheck disable=
function LibCompilerV4.Functions.BytesToHuman()
{
    #**** Parameters ****
...
# shellcheck disable=
function LibCompilerV4.Functions.CheckISO639_1_LangCode()
{
    #**** Parameters ****
...
# shellcheck disable=
function LibCompilerV4.Functions.CheckLangArgDelim()
{
    #**** Parameters ****
...
# shellcheck disable=SC2059
function LibCompilerV4.Functions.HandleIncompatibleOptionalArgs()
{
    #**** Parameters ****
...
# shellcheck disable=
function LibCompilerV4.Functions.CompilerUsage()
{
    #**** Parameters ****
    local p_exit=${1:-true};
...
# shellcheck disable=SC2059
function LibCompilerV4.Functions.QuitErrorMessage_NumberOfFilesCompiled()
{
...
# shellcheck disable=
function LibCompilerV4.PreCompilationFunctions.OptionalArgumentsArrayLooping()
{
    ...
    for arg in "${__BU__BIN__LIB_COMPILER_V4__ARGS__OPTIONS_ARRAY[@]}"; do
        ...
        # Else, if the user decides to get a quick help about the compiler.
            ...
            LibCompilerV4.Functions.CompilerUsage;
...
# shellcheck disable=
function LibCompilerV4.PreCompilationFunctions.CheckForAnyIncompatibleOptionalArgs()
{
    ...
    if [ -n "${__vArrayVal_keep_raw_document_layout}" ]; then
        ...
        then
            LibCompilerV4.Functions.HandleIncompatibleOptionalArgs \
...
function LibCompilerV4.CompileInSingleFile()
{
    ...
    # Parsing the array of optional arguments given by the user.
    LibCompilerV4.PreCompilationFunctions.OptionalArgumentsArrayLooping;

    # Checking if any incompatible optional values were passed together during the execution of the compiler.
    LibCompilerV4.PreCompilationFunctions.CheckForAnyIncompatibleOptionalArgs;

    # Processing the authorization to skip the verification of a file before its compilation.
    LibCompilerV4.PreCompilationFunctions.ManageShellcheck;
    ...
    # Converting the "${p_locale}" string into an array of ISO 639-1 codes.
        ...
        if      LibCompilerV4.Functions.CheckLangArgDelim "${p_locale}" ','; then v_locale_delim=',';
        elif    LibCompilerV4.Functions.CheckLangArgDelim "${p_locale}" ';'; then v_locale_delim=';';
        elif    LibCompilerV4.Functions.CheckLangArgDelim "${p_locale}" '|'; then v_locale_delim='|';
        elif    LibCompilerV4.Functions.CheckLangArgDelim "${p_locale}" '~'; then v_locale_delim='~';
        elif    LibCompilerV4.Functions.CheckLangArgDelim "${p_locale}" '_'; then v_locale_delim='_';
        elif    LibCompilerV4.Functions.CheckLangArgDelim "${p_locale}" '.'; then v_locale_delim='.';
        elif    LibCompilerV4.Functions.CheckLangArgDelim "${p_locale}" ':'; then v_locale_delim=':';
        elif    LibCompilerV4.Functions.CheckLangArgDelim "${p_locale}" '!'; then v_locale_delim='!';
        elif    LibCompilerV4.Functions.CheckLangArgDelim "${p_locale}" '§'; then v_locale_delim='§';
    ...
    if [ "${v_is_check_to_do}" == 'true' ]; then
        if [ "${p_locale,,}" == 'all' ]; then
            LibCompilerV4.Functions.PrintWarningLine "${__HIGHLIGHT}${p_locale}${__WARNING} | ${__HIGHLIGHT}lang=all${__WARNING} : THIS FEATURE IS NOT YET FULLY IMPLEMENTED, WATCH OUT FOR THE BUGS" 'FULL';
    ...
    for language in "${__language_array[@]}"; do
        if [ "${__BU__BIN__LIB_COMPILER_V4__NUMBERS__COMPILER_LOOP_ITERATOR}" -eq 0 ]; then
            #------------------------
            #**** Loop variables ****
            declare -g __BU__LIB_COMPILER_V4_FULL_FILE_PATH; # VAR TYPE : Filepath   - DESC : Path to the temporary file which stores the output of each file.
            ...
            function LibCompilerV4.CompileInSingleFile.Loop.IsCurrentLanguageEnglish()
            {
        ...
        __BU__LIB_COMPILER_V4_FULL_FILE_PATH="${__v_loop_compiled_file_parent_dir}/Bash-utils-compilation-output.tmp";

        # TODO : separate the declaration of the variables which have to be redeclared for each loop iteration.
        # readonly __BU__LIB_COMPILER_V4_FULL_FILE_PATH;
        ...
        if ! LibCompilerV4.Functions.CheckISO639_1_LangCode "${__v_loop_curr_locale}"; then
            LibCompilerV4.Functions.PrintErrorLine "${__v_loop_locale_print_code__error} ${__BU__BIN__LIB_COMPILER_V4__GLOBVARS__MSG_E__BAD_LANGUAGE_PASSED}" 'FULL';
        ...
        LibCompilerV4.Functions.PrintNewstepLine "$(printf "${__v_loop_locale_print_code__newstep} ${__BU__BIN__LIB_COMPILER_V4__GLOBVARS__MSG_N__BEGIN_FRAMEWORK_COMPILATION}" "${__BU__LIB_COMPILER_V4_FULL_FILE_PATH}")" 'FULL';
        ...
        if ! LibCompilerV4.CompileInSingleFile.Loop.IsCurrentLanguageEnglish; then
        ...
        if [ "${____no_english_included,,}" == 'true' ]; then
            ...
            if [ ! -f "${__v_loop_locale_file_path_en}" ]; then
                ...
                LibCompilerV4.Functions.WriteBU "${__v_loop_locale_file_path_en}" "${__vArrayVal_display}" || local ____err="error";
            fi
        ...
        if [ -n "${__vMandatoryArgLang}" ]; then
            ...
            if  [ ! -f "${__v_loop_locale_file_path}" ]; then
                ...
                LibCompilerV4.Functions.WriteBU "${__v_loop_locale_file_path}" "${__vArrayVal_display}" || local ____err="error";
            fi
...
#                 'UPPER';
            ...
            for translationLanguage in "${__language_array[@]}"; do
                ...
                if  [ ! -f "${translationFilePath}" ]; then
                    ...
                    LibCompilerV4.Functions.WriteBU "${translationFilePath}" "${__vArrayVal_display}" || local ____err="error";
                fi
        ...
        if [ ! -f "${__BU_INITIALIZER_SCRIPT_PATH}" ]; then
            ...
            LibCompilerV4.Functions.WriteBU "${__BU_INITIALIZER_SCRIPT_PATH}" "${__vArrayVal_display}";
        fi
        ...
        cp "${__BU__LIB_COMPILER_V4_FULL_FILE_PATH}" "${__v_loop_compiled_file_path}" || {
            LibCompilerV4.Functions.PrintErrorLine \
                "$(printf \
        ...
        if [ -z "$__vArrayVal_keep_raw_document_layout" ]; then
            ...
            if [ -z "${__vArrayVal_keep_exec_safeguards}" ]; then LibCompilerV4.Functions.EraseSafeguardExecLines; fi
            ...
            if [ -z "${__vArrayVal_keep_comments}" ]; then LibCompilerV4.Functions.EraseComments; fi

            # Calling the "()" function in order to write the description of the compiled file at its very beginning.
            # if [ -z "${new value to define}" ]; then LibCompilerV4.Functions.WriteCommentCode; fi

            LibCompilerV4.Functions.WriteCommentCode;
        fi
        ...
        printf "${__BU__BIN__LIB_COMPILER_V4__GLOBVARS__MSG_M__LOCALIZED_FILE__BYTES}\n" "$(LibCompilerV4.Functions.BytesToHuman "$(wc -c < "${__v_loop_compiled_file_path}")" 'NULL' 1000 "${____sys_lang}")";
        ...
        if [ -n "${__vArrayVal_compile_stable}" ]; then
            ...
            LibCompilerV4.Functions.PrintNewstepLine "$(printf "${__BU__BIN__LIB_COMPILER_V4__GLOBVARS__MSG_N__COPY_COMPILED_FILE_IN_STABLE_DIRECTORY__CHECKING_ERRORS}" "${__v_loop_compiled_file_path}")" 'UPPER';
            ...
            LibCompilerV4.Functions.PrintNewstepLine "$(printf "${__BU__BIN__LIB_COMPILER_V4__GLOBVARS__MSG_N__COPY_COMPILED_FILE_IN_STABLE_DIRECTORY__CHMOD}" "${__v_loop_compiled_file_path}")" 'UPPER';

            if ! chmod --verbose -xw "${__v_loop_compiled_stable_file_path}"; then
                LibCompilerV4.Functions.PrintWarningLine "$(printf "${__BU__BIN__LIB_COMPILER_V4__GLOBVARS__MSG_W__COPY_COMPILED_FILE_IN_STABLE_DIRECTORY__CHMOD__WARNING}" "${__v_loop_compiled_stable_file_path}")";
                    ...
                    '2' | *)
                        ...
                        if [ "${#__BU_ARRAY__READ_ONLY_FAILED_FILES[@]}" -gt 0 ]; then
                            if [ "${#__BU_ARRAY__READ_ONLY_FAILED_FILES[@]}" -eq 1 ]; then
                                ...
                                LibCompilerV4.Functions.PrintFilesWhichWereNotChmoded "${__v_loop_compiled_stable_file_parent_dir}";
                            fi
                        fi
                ...
                LibCompilerV4.Functions.PrintWarningLine "$(printf "${__BU__BIN__LIB_COMPILER_V4__GLOBVARS__MSG_W__COMPILED_STABLE_FILE_WAS_NOT_SET_IN_READ_ONLY_MODE}" "${__v_loop_compiled_stable_file_path}")" 'LOWER';
            else
                LibCompilerV4.Functions.PrintSuccessLine "$(printf "${__BU__BIN__LIB_COMPILER_V4__GLOBVARS__MSG_S__COMPILED_STABLE_FILE_WAS_SUCCESSFULLY_SET_IN_READ_ONLY_MODE}" "${__v_loop_compiled_stable_file_path}")" 'LOWER';
            fi

            LibCompilerV4.Functions.PrintSuccessLine "$(printf "${__v_loop_locale_print_code__success} ${__BU__BIN__LIB_COMPILER_V4__GLOBVARS__MSG_S__STABLE_COMPILED_FILE_IS_READY_TO_BE_USED}" "${__v_loop_compiled_stable_file_path}")" 'FULL';
